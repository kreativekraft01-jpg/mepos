...
import type { PaymentSplit } from '../../types';import { Tooltip, TooltipTrigger, TooltipContent } from './ui/tooltip';
import type { CustomerResult } from './SearchResults';
import { motion, AnimatePresence } from 'motion/react';
import { useSound } from '../hooks/useSound';

/** Rows are merged when they share the same product (boxId/SKU), transaction type and price. */
const mergeKey = (item: TransactionItem) => `${item.boxId || item.boxName}|${item.type}|${item.price}`;

/** Group duplicate rows (same product/type/price) into a single row with summed qty. Serial-tracked items stay separate. */
function mergeItems(items: TransactionItem[]): TransactionItem[] {
  ...
  for (const item of items) {
    ...
    if (idx !== undefined) {
      out[idx] = { ...out[idx], qty: out[idx].qty + item.qty }
    } else {
...
/** Expand a grouped row into one individual row per unit (each qty 1, unique id). Serial-tracked rows stay as-is. */
function expandItems(items: TransactionItem[]): TransactionItem[] {
  const out: TransactionItem[] = []
  for (const item of items) {
    if (item.requiresSerial || item.qty <= 1) {
      out.push(item)
      continue
    }
    for (let i = 0; i < item.qty; i++) {
      out.push({
        ...item,
        id: `${item.id}-row-${i}-${Math.random().toString(36).slice(2, 7)}`,
        qty: 1
      })
    }
  }
...
interface CartPageProps {
  ...
  onTransactionComplete?: (payments: PaymentSplit[]) => void;
  onSaveTransaction?: (items: TransactionItem[]) => void;
}

export function CartPage({ onBack, onActivateCustomerSearch, initialItems = [], pendingCartItem, setPendingCartItem, selectedCustomer, onAttachCustomer, onTransactionComplete, onSaveTransaction }: CartPageProps) {
  ...
  const [deleteDialog, setDeleteDialog] = useState<'all' | 'selected' | null>(null);
  ...
  useEffect(() => {
    if (pendingCartItem && setPendingCartItem) {
      ...
      setItems((prev) => {
        // In grouped view, the same product/type/price merges into the existing row (qty + 1)
        if (groupedView && !itemWithUniqueId.requiresSerial) {
          const key = mergeKey(itemWithUniqueId);
          const existing = prev.find((e) => mergeKey(e) === key);
          if (existing) {
            return prev.map((e) =>
  ...
  }, [pendingCartItem, setPendingCartItem, playBeep, groupedView]);

  // Toggle between grouped (tidy) and individual rows
  const handleTidyToggle = () => {
    const newItems = groupedView ? expandItems(items) : mergeItems(items);
  ...
  return (
    <div className="flex-1 bg-background flex flex-col overflow-hidden transition-all duration-300 relative">
                  ...
                  <div className="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          onClick={() => onSaveTransaction?.(items)}
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent side="bottom">
                        <p>{groupedView ? 'Tidy Row (grouped)' : 'Show individual rows'}</p>
                      </TooltipContent>
                    </Tooltip>
      ...
      <PaymentDue
        ...
        onComplete={(payments) => {
          onTransactionComplete?.(payments);
      ...
      <ConfirmDialog
        ...
        title={deleteDialog === 'all' ? 'Delete All Items' : 'Delete Selected Items'}
        message={
          deleteDialog === 'all'
            ? 'Are you sure you want to delete all items? This will remove every item from your transaction.'
            : `Are you sure you want to delete ${selectedItems.length} selected item(s)? This will remove them from your transaction.`
        }
        confirmLabel="Delete"
        danger
