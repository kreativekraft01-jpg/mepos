...
export function SearchResults({ 
  ...
  // Category filter options derived from the actual inventory (falls back to common ones).
  const categoryOptions = [...new Set(boxes.map((b) => b.category.trim()).filter(Boolean))];
  const CATEGORY_FALLBACK = ['gaming', 'computing', 'phones', 'audio'];
  const filterCategories = categoryOptions.length > 0 ? categoryOptions : CATEGORY_FALLBACK;
  ...
  const hasResults = customersOnly
    ? filteredCustomers.length > 0
    : filteredBoxes.length > 0 || filteredCustomers.length > 0 || filteredOrders.length > 0;
      ...
      <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
        {!hasQuery ? (
          <div className="p-8 text-center text-muted-foreground">
            <p className="text-base">{customersOnly ? 'Start typing to search for customers...' : 'Start typing to search for boxes, customers, or orders...'}</p>
          </div>
        ) : !hasResults ? (
          <div className="p-8 text-center text-muted-foreground">
          ...
          <div>
            {/* Box Results */}
            {!customersOnly && filteredBoxes.length > 0 && (
            ...
            {/* Order Results */}
            {!customersOnly && filteredOrders.length > 0 && (
              <div>
