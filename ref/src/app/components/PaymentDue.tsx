...
type PaymentMethod = 'cash' | 'card' | 'voucher' | 'cheque' | 'upi' | 'bank_transfer' | 'gift_card';
...
export function PaymentDue({
  ...
  const updatePaymentAmount = (method: PaymentMethod, value: string) => {
    setPaymentAmounts({
      ...paymentAmounts,
      [method]: value
    });
                        ...
                        <div key={method} className="flex items-center gap-3 bg-card p-1 pr-4 border border-border">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center bg-background border border-border">
                              <button
                                onClick={() => {
                                  const current = parseFloat(paymentAmounts[method] || '0');
                                  const newVal = Math.max(0, current - 1);
                                  updatePaymentAmount(method, newVal.toFixed(2));
                                }}
                                disabled={isSuccess}
